...
def login_user(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            return render(
                request, "Login.html", {"errormsg": "Invalid username or password"}
            )
...
def register_user(request):
    ...
    if request.method == "POST":
        ...
        if password1 != password2:
            return render(
                request, "Register.html", {"errormsg": "Passwords do not match"}
            )

        if User.objects.filter(username=email).exists():
            return render(
                request, "Register.html", {"errormsg": "Email is already registered."}
            )
...
def add_to_cart(request):
    if request.method == "POST":
        ...
        cart_item.save()
        return JsonResponse(
            {"message": "Added to cart", "quantity": cart_item.quantity}
        )
...
def make_payment(request):
    ...
    new_order, created = Order.objects.get_or_create(
        user=request.user, order_id=order["id"], defaults={"amount": grand_total}
    )
...
def payment_done(request):
    if request.method == "POST":
        ...
        print(
            f"Payment Callback Received: Order ID - {order_id}, Payment ID - {payment_id}"
        )  # Debugging

        if not order_id or not payment_id:
            return JsonResponse(
                {"error": "Invalid payment details received."}, status=400
            )
